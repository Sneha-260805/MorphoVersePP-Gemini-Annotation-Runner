# Teammate 2 instructions

Your assignment: `assignments/teammate_2.csv` — **521 poems** across
19 of the 20 authorized languages. No Sanskrit, no pilot poem
...
git checkout -b teammate-2/gemini-generation
...
`preflight.py` should report `Assignment 2: 521` among the three
...
python -m morphoverse_gemini_pipeline.delivery.poem_annotator.corpus_gemini_runner_v1_1 `
  --assignment assignments/teammate_2.csv `
  --dry-run
```
...
check `checkpoints/MV++_0002.json` exists.

If the smoke-test poem does **not** pass the stop gate, stop and read
`TROUBLESHOOTING.md` / `reports/failures/MV++_0002.json` before
